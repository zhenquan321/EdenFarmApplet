...
Page({
  ...
  formId: function(options) {
    ...
    // var url = this.baseApiUrl + "?g=Api&m=Test&a=index&form_id=" + options;
    // util.ajax({
    //   url : url,
    //   method : "get",
  ...
  mobile_function: function(e) {
    ...
    if (!/^(((13[0-9]{1})|(15[0-9]{1})|(18[0-9]{1}))+\d{8})$/.test(e.detail.value.trim())) {
  ...
  listenFormSubmit: function(e) {
    // var url = this.baseApiUrl + "?g=Api&m=Test&a=index";
    // util.ajax({
    //   url : url,
    ...
    if (!/^(((13[0-9]{1})|(15[0-9]{1})|(18[0-9]{1}))+\d{8})$/.test(formData.mobile.trim())) {
      ...
      success: function(data) {
        if (data.result == 'ok') {
          ...
          if (self.options.goods_id != undefined && self.options.sell_type != undefined) {
            url = '../../pages/orders/checkout' + "?sell_type=" + self.options.sell_type + "&goods_id=" +
              self.options.goods_id + "&address_id=" + data.address_id;
          }
          wx.redirectTo({
            url: url
          })
      ...
      util.ajax({
        ...
        success: function(data) {
          if (data.result == 'ok') {
            ...
            self.init();
            self.setData({
              "provinceTop": -2 * provinceVal + "em",
              'cityTop': -2 * cityVal + "em",
              'areaTop': -2 * areaVal + "em"
            });
          }
        },
        error: function(res) {
  ...
  choseLocation: function() {
    ...
    wx.chooseLocation({
      success: function(res) {
        that.data.address.address = res.address;
        that.setData({
  ...
  init: function() {
    ...
    this.animationObj1.translate3d(0, provincePos + "em", 0).step({
      duration: 200
    });
    this.setData({
    ...
    this.animationObj2.translate3d(0, cityPos + "em", 0).step({
      duration: 200
    });
    this.setData({
    ...
    this.animationObj3.translate3d(0, areaPos + "em", 0).step({
      duration: 200
    });
    this.setData({
  ...
  touchmove: function(e) {
    ...
    var f = (target["new_" + target.id] - target["old_" + target.id]) * 30 / e.touches[0].clientY;
  ...
  rollGear: function(target) {
    ...
    function setGear(target, val) {
      ...
      switch (target.dataset.type) {
        case "provinces":
          ...
          _that.animationObj2.translate3d(0, cityVal * -2 + "em", 0).step({
            duration: 200
          });
          _that.setData({
          ...
          _that.animationObj3.translate3d(0, areaVal * -2 + "em", 0).step({
            duration: 200
          });
          _that.setData({
    ...
    target["int_" + target.id] = setInterval(function() {
      ...
      if (target.dataset.type == 'provinces') {
        _that.animationObj1.translate3d(0, pos + "em", 0).step({
          duration: 200
        });
        _that.setData({
        ...
        _that.animationObj2.translate3d(0, pos + "em", 0).step({
          duration: 200
        });
        _that.setData({
          animationCitys: _that.animationObj2.export(),
        ...
        _that.animationObj3.translate3d(0, pos + "em", 0).step({
          duration: 200
        });
        _that.setData({
          animationAreas: _that.animationObj3.export(),
