...
Page({
  ...
  onShow: function () {
    ...
    if (shopCarInfoMem && shopCarInfoMem.carts) {
      carts = shopCarInfoMem.carts
    }
    this.data.goodsList.list = carts;
  ...
  totalPrice: function () {
    ...
    for (var i = 0; i < list.length; i++) {
      var curItem = list[i];
      if (curItem.active) {
        if (curItem.group_price!=0)
        {
        total += parseFloat(curItem.group_price) * curItem.quantity;
        }
        else{
          
          total += parseFloat(curItem.market_price) * curItem.quantity;
        }
      }
    }
  ...
  jiaBtnTap: function (e) {
    ...
    var list = this.data.goodsList.list;
    if (index !== "" && index != null) {
      if (list[parseInt(index)].quantity < 100) {
  ...
  gotoOrder: function () {
    ...
    var that = this;
    wx.navigateTo({
      url: '../../../pages/order/check/index?amount=' + that.data.goodsList.totalPrice + '&express_fee=20&first=1',
    });
    wx.hideLoading();
  },
